...
// v2.76 — Modales y paneles condicionales cargan a demanda.
// Evitan meter código pesado (CSV parser, CasoPruebaCard tree) al bundle inicial.
const CSVImportModal = dynamic(
  () => import("@/components/dashboard/historias").then(m => ({ default: m.CSVImportModal })),
  { ssr: false }
)
const CSVImportCasosModal = dynamic(
  () => import("@/components/dashboard/casos").then(m => ({ default: m.CSVImportCasosModal })),
  { ssr: false }
)
const HUForm = dynamic(
  () => import("@/components/dashboard/historias").then(m => ({ default: m.HUForm })),
  { ssr: false }
)
const HistoriaUsuarioDetail = dynamic(
  () => import("@/components/dashboard/historias").then(m => ({ default: m.HistoriaUsuarioDetail })),
  { ssr: false }
)
...
// ═══════════════════════════════════════════════════════════
//  DASHBOARD CLIENT BODY (v2.80)
//
//  app/page.tsx es ahora un Server Component que hace auth check
//  server-side y decide renderizar este body client (si hay sesión
//  válida) o `<LoginScreen />` directo (si no — 0 JS dashboard).
//
//  Cuando el server nos monta ya sabe que el usuario está
//  autenticado; mantenemos los guards defensivos (skeleton +
//  fallback a LoginScreen) por si la sesión expira mid-session.
// ═══════════════════════════════════════════════════════════
